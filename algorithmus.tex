...
Sei $e = \{x, y\}$ eine Kante inzident zu $y$ und $\lambda_{y}(e)$ die Nachricht, die über $e$ nach $y$ geschickt wird. Ist $x$ ein Blatt des Baumes, so gilt $\lambda_{y}(e) = \omega(x)$. Ansonsten seien $z_{1}, ..., z_{deg(x)-1}$ die Nachbarn von $x$ ohne $y$, wobei $deg(x)$ der Knotengrad von $x$ ist.  Außerdem gilt $l_{i} = \lambda_{x}(\{x, z_{i}\})$ und sei o.B.d.A. $l_{i} \geq l_{i+1}$: $$\lambda_{y}(e) = max \{l_{1}, l_{2} + \omega(x)\}$$
